...
def testScatter(ax=None):
    ...
    myplt.scatter(ax,
                  10**(np.random.random(100)*2 -
                       2), 10**(np.random.random(100)*2 - 2),
                  xlog=True, ylog=True,
                  s=(0.1 + np.random.random(100)) * 20,
                  c='C1', label='sober points',
...
def testErrorbar(ax=None):
    ...
    myplt.dataPlot(ax.errorbar, ax, x, y, yerr=dy,
...
def testVectorPlot2d(ax=None):
    ...
    vortices = [n * a + m *
                b for n in range(-rnv, rnv) for m in range(-rnv, rnv)]
    vortices = [(x, y) for (x, y) in vortices if -extra_factor <
                x < extra_factor and -extra_factor < y < extra_factor]
    sx, sy = (1000, 1000)
