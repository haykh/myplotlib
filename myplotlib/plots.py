...
def __setAxLims(ax, coords, log, pad, lim, spines):
    ...
    if log: 
        ...
        p1, p2 = lim
        func_setlim(
            *list(map(lambda p: 10**p, __stretch(np.log10(p1), np.log10(p2), 1.0 + pad))))
    else:
        p1, p2 = lim
...
def __checkDimensions2d(x, y, zz):
    x, y, zz = (np.array(np.squeeze(x)), np.array(
        np.squeeze(y)), np.array(np.squeeze(zz)))
    readShapes = f"`x.shape={x.shape}`, `y.shape={y.shape}`, `zz.shape={zz.shape}`"
    assert len(x.shape) == len(
        y.shape), f"Shapes of `x` and `y` must be of the same dimension: {readShapes}."
    if (len(x.shape) > 1):
        x = x[0, ...]
    if (len(y.shape) > 1):
        y = y[..., 0]
    ...
    assert zz.shape[1] == x.shape[
        0], f"incompatible dimensions between `x` and `zz`: {readShapes}."
    assert zz.shape[0] == y.shape[
        0], f"incompatible dimensions between `y` and `zz`: {readShapes}."
    return (x, y, zz)
...
def __findExtent(x, y, centering):
    if centering == 'edge':
        ...
        dx = (x[1] - x[0])
        dy = (y[1] - y[0])
        extent = (x.min() - dx*0.5, x.max() + dx*0.5,
                  y.min() - dy*0.5, y.max() + dy*0.5)
    else:
        raise ValueError
    return extent
...
def plot2d(ax, x, y, zz,
    ...
    **kwargs .................... : standard matplotlib kwargs passed to `ax.scatter`
    ...
    __setAxLims(ax, np.linspace(
        extent[0], extent[1]), False, padx, xlim, 'bottom')
    __setAxLims(ax, np.linspace(
        extent[2], extent[3]), False, pady, ylim, 'left')
    if cbar is not None:
...
def plotVectorField(ax, x, y, fx, fy, background=None,
    ...
    **kwargs .................... : standard matplotlib kwargs passed to `ax.scatter`
    ...
    weights = img
    _ = (np.sign(weights - np.average(weights)) *
         np.sqrt(np.abs(weights - np.average(weights))))
